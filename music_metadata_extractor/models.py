...
class BaseProviderInput:
    """
    Base input class whose object is to be sent to provider as input.

    Having a base class here is mostly for syntactic purposes to unify the different
...
class BaseProvider(ABC):
    ...
    def __init__(self, input: BaseProviderInput):
        ...
        if isinstance(input, StringInput):
            self.data: ProviderData = self.handle_string_input(
                input.title_string)
            ...
            self.data: ProviderData = self.handle_provider_input(
                input.provider_url)
        else:
